...
import { getProximity, graphql, capitalize, buildFromEstates } from './utils'
...
export async function createApiComponent(components: {
  ...
  function buildTile(fragment: ParcelFragment): Tile {
    ...
    const x = parseInt(searchParcelX) - 150
    const y = parseInt(searchParcelY) - 150
    const id = coordsToId(x, y)
    const name = (estate && estate.name) || parcelName
    const owner = (estate && estate.owner) || parcelOwner
    // const activeOrder = false
    const updatedAt = Math.max(
      estate ? parseInt(estate.updatedAt, 10) : 0,
    ...
    if (estate?.id) {
      tile.estateId = estate.id.split('-').pop()! // estate-0xdeadbeef-<id>
    }
  ...
  function buildEstate(fragment: ParcelFragment): NFT | null {
    const { estate } = fragment
    if (!estate) return null

    const { size, parcels, id: tokenId, name, description } = estate
    const attributes: Attribute[] = [
      {
        trait_type: 'Size',
        value: size,
